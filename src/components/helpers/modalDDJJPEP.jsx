...
export default function ModalDDJJPEP({ isOpen, onClose, datosCliente, onDocumentoGenerado }) {
  ...
  const [caracter, setCaracter] = useState('Firmante');
  ...
  useEffect(() => {
    if (datosCliente) {
      setNombre(datosCliente.firmante || datosCliente.nombre || '');
      setDni(datosCliente.dni || datosCliente.cuit || '');
      setDomicilio(datosCliente.domicilio || '');
      setCaracter(datosCliente.caracter || datosCliente.cargo || 'Representante Legal');
      setFirmaTipeada(datosCliente.firmante || datosCliente.nombre || '');
    }
  }, [datosCliente, isOpen]);

  if (!isOpen) return null;

  // Lógica Canvas Nativa para la Firma Dibujada
  ...
  const handleGenerarPDF = async (e) => {
    ...
    try {
      ...
      page1.drawText('DECLARACION JURADA SOBRE LA CONDICION DE', {
        x: 100, y, size: 12, font: fontBold, color: rgb(0, 0, 0)
      });
      y -= 16;
      ...
      page1.drawText('(LEY N. 25.246 y modif., RESOLUCION UIF N. 35/2023)', {
        x: 120, y, size: 9, font: fontBold, color: rgb(0.3, 0.3, 0.3)
      });
      y -= 30;
      ...
      const textoComparece = limpiarTextoPDF(
        `Comparece el/la Sr./Sra. ${nombre.toUpperCase()}, D.N.I. N. ${dni}, Estado Civil ${estadoCivil}, con domicilio real en ${domicilio}, en caracter de ${caracter.toUpperCase()}, quien suscribe con caracter de FORMAL DECLARACION JURADA y respetuosamente dice:`
      );
      ...
      page2.drawText('Nomina de Personas Expuestas Politicamente (RESOLUCION UIF N. 35/2023)', {
        x: 50, y: y2, size: 11, font: fontBold
      });
      y2 -= 20;

      const textoNomina = [
        "1.- Son consideradas Personas Expuestas Politicamente Extranjeras los funcionarios publicos pertenecientes a paises extranjeros que desempenen o hayan desempenado funciones ejecutivas, legislativas, judiciales o de representacion internacional de alto rango.",
        ...
        "4.- Sin perjuicio de lo expuesto, son consideradas Personas Expuestas Politicamente las autoridades de partidos politicos, autoridades de organizaciones sindicales y empresariales, y administradores de fondos publicos o de la seguridad social.",
      ...
      textoNomina.forEach(item => {
        splitText(item, 95).forEach(line => {
          page2.drawText(line, { x: 50, y: y2, size: 8, font: fontRegular, color: rgb(0.2, 0.2, 0.2) });
          y2 -= 11;
        });
        y2 -= 6;
      });
      ...
      console.error("Error generando PDF PEP:", error);
      alert("Error al generar la Declaracion Jurada PEP. Revisa la consola.");
    }
  };
    ...
    >
      ...
      >
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <h3 style={{ margin: 0, color: '#0284c7' }}>📝 Generar DDJJ Persona Expuesta Políticamente (PEP)</h3>
        ...
        <form onSubmit={handleGenerarPDF} style={{ marginTop: '15px' }}>
          <p style={{ fontSize: '0.85rem', color: '#64748b', marginBottom: '15px' }}>
            Completá los datos del declarante. Podés emitir múltiples declaraciones para distintos integrantes.
          </p>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginBottom: '15px' }}>
            ...
            <div>
              <label style={{ display: 'block', fontSize: '12px', fontWeight: 'bold' }}>Carácter / Cargo</label>
              <input type="text" value={caracter} onChange={(e) => setCaracter(e.target.value)} style={{ width: '100%', padding: '6px' }} />
            </div>
